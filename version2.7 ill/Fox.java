...
public class Fox extends Animal
{
     ...
     */
    public Fox(boolean randomAge, Field field, Location location,String sex,boolean ill)
    {
    ...
    public void act_foggy(List<Animal> newFoxes)
    {
        incrementAge();
        incrementHunger();
        if(isAlive()) {
            giveBirth(newFoxes);            
            // Move towards a source of food if found.
            Location newLocation = findFood_Fog();
            if(newLocation == null) { 
                // No food found - try to move to a free location.
                newLocation = getField().freeAdjacentLocation(getLocation());
            }
    ...
    private Location findFood_Fog()
    {
        Field field = getField();
        List<Location> adjacent = field.adjacentLocations_Fog(getLocation());
        Iterator<Location> it = adjacent.iterator();
            ...
            if(animal1 instanceof Fox&&animal2 instanceof Fox)
            {
                ...
                if(!fox1.getSex().equals(fox2.getSex()))
                {
                    ...
                    for(int b = 0; b < births && free.size() > 0; b++) {
                        ...
                        Fox young = new Fox(false, field, loc," ",false);
                        young.getGender();
                        newFoxes.add(young);
                    }
                }
